                    ...
                    <nav>
                        <ol className="flex flex-col items-center justify-center">
                            {keySearch?.trim().length > 0 ? (
                                <span className="uppercase mx-2 text-[#676e7b]">Tìm kiếm : {keySearch}</span>) :
                                (<></>)}
                            {searchParams.get('category')?.trim().length > 0 ? (
                                <span className="uppercase mx-2 text-[#676e7b]">Danh mục : {getCategoryName(searchParams.get('category'))}</span>) :
                                (<></>)}
                        </ol>
                    </nav>
                </div>
                    ...
                    <div className="grid grid-cols-12 gap-[30px]">
                            ...
                            {/* <div className="wdiget widget_search">
                                <div className="bg-[#F8F8F8] flex  rounded-md shadow-e1 items-center py-[4px] pl-3 relative">
                                    <div className="flex-1">
                                        <input type="text" placeholder="Tìm kiếm khoá học..." className="border-none focus:ring-0 bg-transparent" />
                                    </div>
                                    <div className="flex-none">
                                        <button className="btn-primary rounded-md bg-[#4cbdff] text-[white] py-4 px-8 inline-block text-base leading-6 font-medium transition-all ease-in-out duration-300">
                                            <FontAwesomeIcon icon={faMagnifyingGlass} className="" /></button>
                                    </div>
                                </div>
                            </div> */}
                            <div className="wdiget widget_catagory">
                                <h4 className="text-[22px] leading-8 font-bold text-[#555] mb-8">Danh mục</h4>
                        ...
                        <div className="lg:col-span-9 col-span-12">
                            {loading ? (
                                ...
                                <>
                                    <div className="wdiget py-4 flex md:flex-row flex-col items-center mb-8 space-y-6 md:space-y-0 justify-between">
                                        ...
                                        <div className="flex md:flex-row flex-col flex-0 md:flex md:content-center">
                                            {filterNumber > 0 && (
                                        ...
                                        <div className="flex flex-col gap-[30px]">
                                            {courseList?.data.map((course, index) => (
                                                ...
                                                <NavLink
                                                    ...
                                                    <div className="flex-none">
                                                        <div className="h-[159px] w-[260px] rounded relative">
                                                            <img src={course.image_path ? course.image_path : noImg} alt="" className=" w-full h-full object-cover rounded" />
                                                        </div>
                                                    </div>
                                                    <div className="course-content flex-1">
                                                        <div className="flex justify-between">
                                                            <div className="line-clamp-1 flex-1 items-center">
                                                                <h4 className="text-2xl leading-[36px] mb-4 font-bold text-[#333] line-clamp-2">{course.name}</h4>
                                                            </div>
                                                            <div className="text-2xl leading-[36px] ml-2 mb-4 font-bold text-[#4cbdff] items-start flex-col flex">
                                                                {(course?.price_sell > 0) ? (
                                                                    <>
                                                                        <span className="price-sale webkit-fill-available">{(course?.price_sell.toLocaleString())}đ</span>
                                                                        {/* <span className="pl-1 text-[#929292] line-through text-lg webkit-fill-available">600.000đ</span>
                                                        ...
                                                        <div className="flex space-x-6 mb-4">
                                                            <span className="flex items-center space-x-2 text-[#676e7b]">
